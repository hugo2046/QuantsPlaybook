...
from scr.index import IndexBase
...
def normalize_intruments(target_dir: str) -> None:
    logger.info("start normalize instruments...")

    if not Path(target_dir).exists():
        logger.error(f"{target_dir} not exists.")
        return

    instruments: pd.DataFrame = pd.read_csv(target_dir, delimiter="\t", header=None)
    code: str = instruments.iloc[0, 0]
    if code[:2] not in ("SZ", "SH"):
        logger.info("instruments has been normalized.")
        return

    instruments[0] = instruments[0].apply(lambda x: "{1}.{0}".format(x[:2], x[2:]))
    instruments.to_csv(target_dir, index=None, header=None, sep="\t")
    logger.info("end of normalize instruments.")


if __name__ == "__main__":
    fire.Fire(get_instruments, normalize_intruments)
